...
//从excel导入班主任列表
jQuery("#list").jqGrid({
		url:'getClasses.do?type=1',
     	datatype: 'json',    //从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
     	mtype: 'POST',
     	colNames:['ID','编号','级号','名称','班主任','班长','大小','开课日期'],		//表格的显示字段
		colModel:[ 
        ...
        {name:'graduateNum',index:'graduateNum', width:150,editable:true,sorttype:"string",resizable:false},      
        {name:'name',width:150, editable:true,viewable:true,editrules:{edithidden:true}},           
        {name:'headTeacherName',index:'graduateNum', width:150,editable:true,sorttype:"string",resizable:false},      
        {name:'monitorName',index:'monitorName', width:150,editable:true,sorttype:"string",resizable:false},      
        {name:'size',index:'size',width:250,viewable:true},
        {name:'openDate',index:'openDate',width:250,viewable:true}
     	],    
		...
		sortable:true,
		sortname: 'headerNo',    //默认的排序列。可以是列名称或者是一个数字，这个参数会被提交到后台
		viewrecords: true,	//是否显示记录总数
		sortorder: "asc", 
		height: "400px",
		width: 930,   
		rowNum: '9999',
		editurl: 'editClass.do',
 ...
 //edit
        {
    		...
    		bClose: "关闭",  
     		...
     		serializeEditData: function (postdata) {
      			...
      			postdata.directorId = rowdata.directorId;
		      	return postdata;
 			}
        },
        //add
        ...
        //del
        {
        	...
        	serializeDelData: function (postdata) {
      			var rowdata = $("#list").getRowData(postdata.id);
		      	return {directorId: rowdata.directorId, oper: postdata.oper};
 			}
        },
         //search
        {
