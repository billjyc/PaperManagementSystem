...
public class StudentProposalManager {
	...
	private void sendSuggestion(Integer studentId, File proposalfile) {
		...
		String content = sugg_s.getStudentGrade() + "年级" + "学生 " + sugg_s.getStudentName() 
				+ "(学号"+sugg_s.getStudentNumber()+")" + "提交开题报告，附件为开题报告";
		...
		String title = "学生" + sugg_s.getStudentName() + "提交开题报告";
		...
		try {
			...
			attachmentfiles.add(proposalfile);
			emailUtil.sendMailAttach(sugg_t.getTeacherEmail(), title, content, attachmentfiles);
