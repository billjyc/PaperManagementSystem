...
public class StudentPaperManager {
	...
	private void sendSuggestion(Integer studentId, File paperFile, Paper paper) {
		...
		String title = "学生" + sugg_s.getStudentName() + "提交论文";
		String content = sugg_s.getStudentGrade()+"年级"+"学生 "+sugg_s.getStudentName() + 
				"(学号"+sugg_s.getStudentNumber()+")"+"提交论文";
		...
		sugg.setSuggestContent(content);
		...
		sugg.setSuggestTitle(title);
		sugg.setIsRead(false);
		...
		//向导师发送邮件
		sendEmail(sugg_t, title, content, paperFile, paper.getPaperAffix1());
	}
	
	private void sendEmail(Teacher t, String title, String content, File paperFile, String affix) {
		try {
			List<File> attachmentfiles = new ArrayList<>();
			attachmentfiles.add(paperFile);
			content += "<br/>";
			content += "<p>附言：" + affix + "</p>";
			emailUtil.sendMailAttach(t.getTeacherEmail(), title, content, attachmentfiles);
	...
	public JSONObject submitPaper(HttpServletRequest request, String papertitle, String actorId,  
		...
		Integer teacherAccountId = accountService.getAccountByAccountname(
				teacherService.getTeacherById(
						studentService.getStudentById(studentId).getTeacherId())
						.getTeacherNumber()).getAccountId();
		JSONObject jsonobj = new JSONObject();
		jsonobj.put("teacherAccountId", teacherAccountId);
		jsonobj.put("result_json", result_json);
		return jsonobj;
	}
	...
	public JSONObject updatePaper(HttpServletRequest request, String papertitle, 
		...
		String title = "学生" + s.getStudentName() + "更新了论文";
		String content = s.getStudentGrade()+"年级"+"学生 "+s.getStudentName() + 
				"(学号"+s.getStudentNumber()+")"+"更新了论文";
		sendEmail(t, title, content, save, paperaffix1);
