...
public class TeacherPaperController {
	...
	@ResponseBody
	public void getUncheckedPrelimaryList(HttpServletRequest request, HttpServletResponse response,
			ModelMap model){
		HttpSession session = request.getSession();
		String actorNo = (String) session.getAttribute("actorNo");
		JSONObject jsonobj = teacherPaperManager.getUncheckedPrelimaryList(actorNo);
	...
	@ResponseBody
	public void judgePreliminary(HttpServletRequest request, HttpServletResponse response, ModelMap model, 
		...
		JSONObject jsonobj = teacherPaperManager.judgePreliminary(paperIdS, resultS, actorNo, request, taskIdS, addition);
		jsonUtil.sendJson(response, jsonobj);
	}
